...
```bash
curl -X POST http://localhost:5005/api/auth/sign-in/confirm \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST http://localhost:5005/api/identity/generate-encrypted-key \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST http://localhost:5005/api/analytics/start-session \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "user123",
